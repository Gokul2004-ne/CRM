  ...
  const handleSave = () => {
    ...
    if (modal.editing) {
      ...
      if (targetSubIds.length === 0) {
        ...
        toast.success(`Assigned ${targetSubIds.length} service(s) to ${clientName}!`);
      }
    }
                  ...
                  >
                    ...
                    <td className="col-actions">
                      <div style={{ display: "flex", gap: 4 }}>
                        ...
                        <button className="icon-btn-slds" title="Delete" onClick={async () => {
                          try {
                            await deleteAssignedService(a.id);
                            toast.success("Assigned package deleted");
                          } catch (err) {
                            console.error(err);

  ...
  try {
    ...
    const formattedClients: Client[] = (clients || []).map((c: any) => ({
      ...
      portalCredentials: c.gst_portal_id || c.gst_portal_password
        ...
        : [],
      documents: Array.isArray(c.documents) ? c.documents : (typeof c.documents === "string" ? JSON.parse(c.documents) : []),
      documentCount: c.document_count || (Array.isArray(c.documents) ? c.documents.length : 0),
